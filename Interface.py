        ...
        def gegevens(naam):
            ...
            for x in gegevensdata:
                if x[1] == str(naam):
                    ...
                    aanalarm = 'tijd aan: ' + str(x[12])
                    uitalarm = 'tijd aan: ' + str(x[13])
                    alarmactief = 'Actief?: ' + str(x[14])
                    ...
                    alarm_frame = ttk.LabelFrame(gegevens_frame, text="Alarm", relief=tk.RIDGE)
                    alarm_frame.grid(row=6, column=1)
                    ttk.Label(alarm_frame, text=aanalarm).grid(row=1, column=1, sticky=tk.W)
                    ttk.Label(alarm_frame, text=uitalarm).grid(row=2, column=1, sticky=tk.W)
                    ttk.Label(alarm_frame, text=alarmactief).grid(row=3, column=1, sticky=tk.W)
                    ttk.Button(alarm_frame, text='Wijzigen', command=lambda: alarm_wijzigen()).grid(row=4, column=1)

                    tk.Button(gegevens_frame, text='Terug', command=combine3).grid(row=7, column=1)
                    break
            ...
            def alarm_wijzigen():
                ...
                def combine6(een, twee, drie):
                    datawijzigen2(een, twee, drie)
                    gegevens_frame.grid_remove()
                    time.sleep(0.3)
                    gegevens(naam)

                alarmwijzig = ttk.LabelFrame(gegevens_frame, text="Alarm", relief=tk.RIDGE)
                alarmwijzig.grid(row=6, column=1)
                startalarmentry = ttk.Entry(alarmwijzig, width=7)
                startalarmentry.grid(row=1, column=2)
                ttk.Label(alarmwijzig, text='Tijd aan:').grid(row=1, column=1, sticky=tk.W)
                startalarmentry.insert(tk.END, str(x[12]))
                stopalarmentry = ttk.Entry(alarmwijzig, width=7)
                stopalarmentry.grid(row=2, column=2)
                ttk.Label(alarmwijzig, text='Tijd uit:').grid(row=2, column=1, sticky=tk.W)
                stopalarmentry.insert(tk.END, str(x[13]))
                ttk.Button(alarmwijzig, text='Wijzigen', command=lambda: combine6(str(startalarmentry.get()), str(stopalarmentry.get()), str(x[0]))).grid(row=3, column=1)
