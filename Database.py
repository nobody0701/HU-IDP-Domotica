...
def data(datatype):
    ...
    if datatype != str:
        ...
        if datatype == 2:
            cur.execute("SELECT a.*, b.*, c.*, d.* FROM kamer a, stofzuiger b, gordijnen c, alarm d WHERE a.kamerid = b.kamerid AND a.kamerid = c.kamerid;")
        rows = cur.fetchall()
        cur.close()
        return rows
...
def datawijzigen2(data1, data2, data3):
    conn = psycopg2.connect("dbname='idp' user='postgres' host='localhost' password='0000'")
    cur = conn.cursor()
    cur.execute("UPDATE alarm SET starttijd = '" + data1 + "', stoptijd = '" + data2 + "' WHERE kamerid = " + data3 + ";")
